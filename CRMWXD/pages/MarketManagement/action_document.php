...
switch($action){
    case "add":
        ...
        if($name==''){
            $data[code]=1;
            $data[message]=urlencode("文档名称不能为空");
            echo urldecode(json_encode($data));
        }else if($message==''){
            $data[code]=2;
            $data[message]=urlencode("文档描述不能为空");
            echo urldecode(json_encode($data));
        }else if($address==''){
            $data[code]=3;
            $data[message]=urlencode("文档地址不能为空");
            echo urldecode(json_encode($data));
        }else{
            $sql="insert into market_document values(null,'{$name}','{$message}','{$address}','{$addtime}')";
    ...
    case "del":
        ...
        header("Location:MarketDocument.php");
        break;
    case "update":
        $id=$_POST["id"];
        $name=$_POST["name"];
