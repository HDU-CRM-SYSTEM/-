...
switch($action){
    case "add":
        ...
        if($name==''){
            $data[code]=1;
            $data[message]=urlencode("活动名称不能为空");
            echo urldecode(json_encode($data));
        }else if($message==''){
            $data[code]=2;
            $data[message]=urlencode("活动信息不能为空");
            echo urldecode(json_encode($data));
        }else if($address==''){
            $data[code]=3;
            $data[message]=urlencode("活动地址不能为空");
            echo urldecode(json_encode($data));
        }else{
            $sql="insert into market_activity values(null,'{$name}','{$message}','{$address}','{$addtime}')";
    ...
    case "del":
        ...
        header("Location:MarketActivity.php");
        break;
    case "update":
        $id=$_POST["id"];
        $name=$_POST["name"];
